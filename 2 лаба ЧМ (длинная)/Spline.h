...
public:
	...
	// boundary conditions type for the spline end-points
	enum bd_type {
			first_deriv = 1,
			second_deriv = 2,
			not_a_knot = 3
...
public:
	...
	// where a[i] = y[i], or else it won't go through grid points
	...
	void set_coefficients_from_b(); // calculate c_i, d_i from b_i
	size_t find_closest(double x) const; // closest idx so that m_x[idx]<=x

public:

	// defaul constructor: set boundary condition to be zero curvature
	...
	Spline(
		...
		m_made_monotonic(make_monotonic)
	{
...
public:
	...
	std::vector<double> lu_solve(const std::vector<double>& b, bool is_lu_decompose = false);
...
{
	assert(m_x.size() == 0); // set_points() must not happened yet
	m_left = left;
	m_right = right;
	m_left_value = left_value;
	m_right_value = right_value;
...
void Spline::set_coefficients_from_b() {
	size_t n = m_b.size();
	if (m_c.size() != n)
		m_c.resize(n);
	if (m_d.size() != n)
	...
	spline_type type)
{
	...
	m_y = y;
	int n = (int)x.size();
	// check sctrict monotonicity of input vector x
	//...
	...
	if (type == linear) {
		...
		// ignore boudary conditions, set slope equal to the last segment
		m_b[n - 1] = m_b[n - 2];
		m_c[n - 1] = 0.0;
		m_d[n - 1] = 0.0;
	}
		...
		// this requires solving and equation system
		...
		if (m_right == Spline::second_deriv) {
			...
			// (c[n-2] + 2c[n-1])(x[n-1]-x[n-2]) = 3 (f' - (y[n-1] - y[n-2)/(x[n-1] - x[n-2]))
		...
		// solve the equation system to obtain the parameter c[]
		m_c = A.lu_solve(rhs);
		...
		// for the right extrapolation coefficients (sero cubic term)
		// f_{n-1}(x) = y_{n-1} + b*(x - x_{n-1}) + c*(x - x{n-1})^2
...
bool Spline::make_monotonic() {
	...
	for (int i = 0; i < n - 1; i++) {
		...
		if (avg == 0.0 && (m_b[i] != 0.0 || m_b[i + 1] != 0.0)) {
			...
			double r = sqrt(m_b[i] * m_b[i] + m_b[i + 1] * m_b[i + 1]) / fabs(avg);
			if (r > 3.0) {
...
size_t Spline::find_closest(double x) const
{
	...
	it = upper_bound(m_x.begin(), m_x.end(), x);       // *it > x
